...
(defun %parse-false (string index)
  (declare (type simple-string string)
           (type fixnum index))
  (if (and (char= #\a (char string (1+ index)))
           (char= #\l (char string (+ index 2)))
           (char= #\s (char string (+ index 3)))
           (char= #\e (char string (+ index 4))))
      (values nil (incf index 5))
      (error "Expected 'false' at position ~a!" index)))
...
(defun parse (string &optional (index 0) (skip-p t))
  ...
  (let* ((index (if skip-p
    ...
    (cond
      ...
      ((char= character #\t) (%parse-true string index))
      ((char= character #\f) (%parse-false string index))
